const { prefix, hentai } = require('../config.json'); // Load prefix and hentai setting
...
module.exports = {
    ...
    async execute(message, client) {
        ...
        if (fs.existsSync(path.join(__dirname, '..', 'excesscommands', 'hentai', `${commandName}.js`))) {
            if (!hentai) return message.reply('Hentai commands are currently disabled.'); // Check if hentai commands are enabled
